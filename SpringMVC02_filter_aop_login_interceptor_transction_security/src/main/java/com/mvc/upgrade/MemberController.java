...
public class MemberController {
	...
	// security-context.xml에 생성한 객체와 연결
	...
	public Map<String, Boolean> ajaxLogin(HttpSession session, @RequestBody MemberDto dto){
		// HttpSession에 아이디 비밀번호를 담아야하기 때문에 HttpSession 객체를 매개변수로 갖고 온다.
		...
		if(res != null) {// 객체가 있다. -> 로그인 성공
			// db에 저장된 password와 화면에서 입력한 password가 일치하는지 확인
			// 암호화 객체명.matches(암호화되지 않는 pw, 암호화된 pw);
			// 암호화된 비밀번호를 복호화하지 않고 비교한다. (애초에 복호화 메소드 자체는 존재하지 않는다.)
			if(passwordEncoder.matches(dto.getMemberpw(), res.getMemberpw())) { // 비교 결과가 true면
	...
	public String memberInsert(MemberDto dto) {
		...
		// 현재 입력받은 pw 값을 getter로 가져와서 암호화한 후, dto에 setter로 다시 저장한다.
		
		System.out.println(dto.getMemberpw()); // 암호화된 pw 값 출력
		
		int res = biz.insert(dto);
		
		return (res > 0)?"redirect:loginform.do":"redirect:registerform.do";
	}
